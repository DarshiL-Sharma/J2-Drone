...
class VideoStream:
    ...
    def _loop(self):
        ...
        # Force OpenCV to keep only the newest frame internally instead of
        # queueing everything up - this is what causes the growing delay,
        # since read() would otherwise hand back an increasingly stale
        # buffered frame whenever inference falls behind the stream's fps.
        ...
        while self.running:
            # Drain any extra buffered frames (cheap - no decode) so we
            # always end up on the most recent one, then decode only that.
            got_any = False
            for _ in range(5):
                ok = self.cap.grab()
                if not ok:
