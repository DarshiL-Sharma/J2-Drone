...
class VideoStream:
    ...
    def _draw_person_boxes(self, frame, results):
        person_count = 0
    ...
    def _capture_loop(self):
        self.status = "connecting to stream..."
        # Force RTSP over UDP instead of TCP.
        os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = "rtsp_transport;udp"
    ...
    def _process_loop(self):
        ...
        while self.running:
            ...
            try:
                ...
                person_count = self._draw_person_boxes(annotated, results)
                if person_count:
                    now = time.time()
                    if now - self.last_victim_save_time >= VICTIM_SAVE_COOLDOWN_SECONDS:
                        self._save_victim_capture(annotated)
